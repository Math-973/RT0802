...
def on_message(client, userdata, msg):
    ...
    if 'vendor' in message and message['vendor'] == 'Vendor 1' and 'revoked' in message:
        logging.info("Verification response received: %s", message)
        print(f"Client 1 - Verification response: {message}")
        if not message['revoked']:
            ...
            logging.info("Sent purchase request for vendor Vendor 1 by client Client 1")
        else:
            logging.info("Vendor 1's certificate is revoked. Cannot proceed with purchase.")
...
def client_action(vendor_cert):
    ...
    logging.info("Sent verification request for vendor Vendor 1")

if __name__ == "__main__":
    vendor_cert = load_certificate("../vendor1/certs/vendor_1_cert.pem")
